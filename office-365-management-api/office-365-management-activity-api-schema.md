...
|GenericInfo|Edm.String|不要|コメントやその他の一般的な情報に使用されます。|
|||||
...
|Message|Edm.String|はい|このパラメーターには、エンティティに関連して実行された操作が含まれます。 たとえば、新しい連絡先が作成された場合、Message プロパティの値は `Create` となり、EntityName プロパティの対応する値はに `contact` なります。|
